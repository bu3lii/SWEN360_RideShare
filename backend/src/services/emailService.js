...
class EmailService {
  ...
  async sendVerificationEmail(user, verificationToken) {
    const apiBaseUrl = config.frontendUrl.includes('/api') 
      ? config.frontendUrl 
      : `${config.frontendUrl}/api/${config.apiVersion}`;
    const verificationUrl = `${apiBaseUrl}/auth/verify-email/${verificationToken}`;
  ...
  async sendPasswordResetEmail(user, resetToken) {
    const apiBaseUrl = config.frontendUrl.includes('/api') 
      ? config.frontendUrl 
      : `${config.frontendUrl}/api/${config.apiVersion}`;
    const resetUrl = `${apiBaseUrl}/auth/reset-password/${resetToken}`;
