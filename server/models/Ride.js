...
const rideSchema = new mongoose.Schema({
  ...
  // Pricing (calculated after ride completion)
  pricePerSeat: {
    type: Number,
    default: 0,
...
rideSchema.statics.findRidesNearLocation = async function(coordinates, maxDistanceMeters = 5000) {
  const locationService = require('../services/locationService');
  
  // Get all scheduled rides with available seats
  const allRides = await this.find({
    status: 'scheduled',
    availableSeats: { $gt: 0 },
    departureTime: { $gt: new Date() }
  }).populate('driver', 'name rating profilePicture carDetails');
  ...
  for (const ride of allRides) {
    if (ride.startLocation?.coordinates) {
