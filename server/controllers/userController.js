...
exports.getDashboard = asyncHandler(async (req, res, next) => {
  ...
  const [
    ...
    // Upcoming rides as passenger (pending/confirmed, not in progress)
    Booking.find({
      passenger: userId,
    ...
    // Upcoming rides as driver (scheduled only)
    req.user.isDriver
      ? Ride.find({
          driver: userId,
          status: 'scheduled',
    ...
    // Active rides as passenger (in_progress)
    Booking.find({
      passenger: userId,
    ...
    // Active rides as driver (in_progress)
    req.user.isDriver
      ? Ride.find({
          driver: userId,
          status: 'in_progress'
  ...
  const validActiveRidesAsPassenger = activeRidesAsPassenger.filter(b => b.ride);
  ...
  res.status(200).json({
    ...
    data: {
      ...
      upcomingRidesAsDriver,
      activeRideAsPassenger: validActiveRidesAsPassenger.length > 0 ? validActiveRidesAsPassenger[0] : null,
      activeRideAsDriver: activeRidesAsDriver.length > 0 ? activeRidesAsDriver[0] : null,
      recentBookings,
      impact: {
