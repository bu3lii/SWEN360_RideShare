...
exports.getConversations = asyncHandler(async (req, res, next) => {
  ...
  // Add unread count and format participant for each conversation
  const conversationsWithUnread = await Promise.all(
    conversations.map(async (conv) => {
      const unreadCount = await Message.getUnreadCount(req.user._id, conv._id);
      
      // Ensure participants are populated (they should be from getUserConversations)
      await conv.populate('participants', 'name profilePicture');
      ...
      // If participant is still an ObjectId, fetch the user
      let participantData = null;
      if (otherParticipant) {
        if (otherParticipant._id && otherParticipant.name) {
          // Already populated
          participantData = {
...
exports.getConversationMessages = asyncHandler(async (req, res, next) => {
  ...
  const formattedConversation = {
    ...
    isBlocked: conversation.blockedBy != null
  };
